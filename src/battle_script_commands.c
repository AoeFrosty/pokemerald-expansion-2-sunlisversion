...
static void Cmd_attackcanceler(void)
{
    ...
    else if (gSpecialStatuses[gBattlerTarget].shortFuseRedirected)
    {
        gSpecialStatuses[gBattlerTarget].shortFuseRedirected = FALSE;
        gLastUsedAbility = ABILITY_SHORT_FUSE;
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_TookAttack;
        RecordAbilityBattle(gBattlerTarget, gLastUsedAbility);
    }
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    if (gSideStatuses[battlerDef] & SIDE_STATUS_LUCKY_CHANT)
    {
        ...
        || (abilityAtk == ABILITY_SURGE_HUNTER && gBattleMons[battlerDef].status1 & STATUS1_PARALYSIS))
    {
...
void SetMoveEffect(bool32 primary, bool32 certain)
{
    ...
    if (!primary && affectsUser != MOVE_EFFECT_AFFECTS_USER
      ...
      && (battlerAbility == ABILITY_SHIELD_DUST ||  battlerAbility == ABILITY_ROYAL_COAT || GetBattlerHoldEffect(gEffectBattler, TRUE) == HOLD_EFFECT_COVERT_CLOAK))
    {
        if (battlerAbility == ABILITY_SHIELD_DUST || battlerAbility == ABILITY_ROYAL_COAT)
            ...
            if (stats & 1)
            {
                if (flags & STAT_CHANGE_CANT_PREVENT)
                {
                    if (gBattleMons[battler].statStages[currStat] > MIN_STAT_STAGE)
                    {
                        ...
                        && !(ability == ABILITY_EARLY_BIRD && currStat == STAT_SPEED)
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if ((index = IsFlowerVeilProtected(battler)) && !certain)
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
                ...
                || (battlerAbility == ABILITY_EARLY_BIRD && statId == STAT_ATK)
